...
import { getMeasurements, getMeasurementsBySensor } from "../lib/Api";
...
export const Chart2 = ({
  ...
  const sensors = useMemo(() => {
    const map = new Map<
      string,
      { sensorId: string; sensorName: string; measuredDays: string[] }
    >();
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      if (measurementsCache.has(selectedSensor)) return; // TIEDOT ON JO TALLENNETTU CACHEEN
  ...
  // SENSORIN VAIHTUESSA NÄYTETÄÄN KAIKKI PÄIVÄT
  useEffect(() => {
    setSelectedDay("all");
  }, [selectedSensor]);
